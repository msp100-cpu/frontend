...
const Cart = ({ onClose }) => {
  ...
  const { isLoggedIn, cart } = useSelector((store) => store);
  ...
  const onRemove = () => {
    ...
    removeProductFromCart(token, { cartProductID: product.cartItemID })
      .then((res) => {
        let newCartList = data.cartProducts.filter(
          (item) => item.cartItemID !== product.cartItemID
        );
        console.log(newCartList);
        setData({ ...data, cartProducts: newCartList });
      })
      .catch((err) => {});
  };
            ...
            <div className="h-full flex flex-col bg-white shadow-xl overflow-y-scroll">
              ...
              <div className="border-t border-gray-200 py-6 px-4 sm:px-6">
                ...
                <div className="mt-6">
                  <a
                    href="#"
                    className="flex justify-center items-center px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-gray-800 hover:bg-black duration-150"
                  >
                    Checkout
                  </a>
                </div>
